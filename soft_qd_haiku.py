...
def parse_response(response, default=None):
    ...
    try:
        ...
        if isinstance(parsed, dict) and isinstance(parsed.get("quality"), int):
            return parsed
    ...
    try:
        ...
        parsed = json.loads(response)
        # Check if the "quality" field exists and is an integer
        if isinstance(parsed, dict) and isinstance(parsed.get("quality"), int):
            return parsed
    except (json.JSONDecodeError, TypeError, ValueError):
        pass
...
def run_main(
    ...
    """
    key = "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"
    ...
    use_random = True # if true, we will use a random solution instead of CMA-ES/ME/MAE solutions

    # misc data
    import datetime
    current_time = datetime.datetime.now()
    file_name = current_time.strftime("random_results_32_%Y-%m-%d_%H-%M-%S")
    ...
    # create the x0
    inject = "Happy"
    inject_genre = "Sonnet" #TODO: dynamically mutate  word haiku?
    
    full_message = [
        {"role": "system", "content": "You are a helpful assistant"},
        {"role": "user", "content": "create a poem with theme: and genre:"},
        ]
    ...
    inject_genre_input_ids = tokenizer(inject_genre, add_special_tokens=False)['input_ids']

    inject_genre_embeddings = model.get_input_embeddings()(torch.tensor(inject_genre_input_ids))
    print("sizes:")
    print(inject_embeddings.size())
    print(inject_genre_embeddings.shape)
    print(full_message_embeddings.size())
    # concatenate theme and genre embedding. 
    concatenated_embeddings = torch.cat((inject_embeddings, inject_genre_embeddings), dim=0)
    print(concatenated_embeddings.shape)

    x0 = concatenated_embeddings.reshape(-1).float().cpu().detach().numpy()
    ...
    def quality(
        ...
        quality_str = f"""{poem}
Rate the quality of the above poem as a Haiku on a scale from 1 to 10. quality should be 0 if poem is not Haiku. Answer in JSON with the key 'quality'.
"""
...
"""
    ...
    for itr in trange(1, total_itrs + 1, file=sys.stdout, desc='Iterations'):
        ...
        print(f"solution_batch_dim:{solution_batch.shape[1]}")
        ...
        for s in solution_batch:
            ...
            if use_random:
                print("using a embedding with noiseee instead of CMA-ME")
                soft_prompt = torch.tensor(x0, device=model.device, dtype=model.dtype).reshape(1,  s.shape[0] // 4096, -1)
                soft_prompt = soft_prompt + (torch.randn_like(soft_prompt) * 0.05)
                theme_e = soft_prompt[:, 0,: ]  # Shape will be (1, X)
                theme_e = theme_e.unsqueeze(1)
                # Second tensor based on dimension 2,3:
                genre_e = soft_prompt[:, 1:, :]  # Shape will be (1, X, Y-1)
                print(theme_e.shape)
                print(genre_e.shape)
            ...
            print("theme prompt shape:", theme_e.shape)
            print("full size:",full_message_embeddings.shape)
            decoded_segment_before_theme = tokenizer.decode(torch.argmax(full_message_embeddings, dim=-1)[0], skip_special_tokens=False)
            print("Segment Before Theme Insertion:", decoded_segment_before_theme)

            # Insert the mutated embeddings back into the current_message_embeddings
            print("concating newly muatated embedding: ")
            theme_insert_pos = 23 # NOTE: this is based on the prompt.
            genre_insert_pos = 26
            # Insert theme embedding
            injected_input_embeddings = torch.cat(
                [full_message_embeddings[:, :theme_insert_pos, :], 
                theme_e, 
                full_message_embeddings[:, theme_insert_pos:, :]],
                dim=1
            )

            # Insert genre embedding after the theme embedding has been inserted
            injected_input_embeddings = torch.cat(
                [injected_input_embeddings[:, :genre_insert_pos, :], 
                genre_e, 
                injected_input_embeddings[:, genre_insert_pos:, :]],
                dim=1
)           
